...
def cifar_dataset(config):
    ...
    # cifar_dataset_root = '/ddnstor/imu_tssw1/hechao/dataset/'
    cifar_dataset_root = '/data02/WeiHongxi/Node95/hechao/dataset/'
    # cifar_dataset_root = 'E:\\research_topic\\dataset\\'
    # Dataset
...
def compute_result(dataloader, net, code_length, num_class, device):
    bs, clses = [], []
    net.eval()
    with torch.no_grad():
        N = len(dataloader.dataset)
        code = torch.zeros([N, code_length]).to(device)
        cls_code = torch.zeros([N, num_class]).to(device)
        for idx, (img, cls, ind) in enumerate(tqdm(dataloader, ncols=60)):
            ind = ind.to(device)
            out = net(img.to(device))
            code[ind,:] = out.sign()
            cls_code[ind,:] = cls.float().to(device)


    net.train()
    # return torch.cat(bs).sign(), torch.cat(clses)
    return code, cls_code
...
def validate(config, Best_mAP, test_loader, dataset_loader, net, epoch):
    device = config["device"]
    print("calculating test binary code......")
    tst_binary, tst_label = compute_result(test_loader, net, config["bit"], config["n_class"], device=device)

    # print("calculating dataset binary code.......")
    trn_binary, trn_label = compute_result(dataset_loader, net, config["bit"], config["n_class"], device=device)

    mAP, index = CalcTopMap(trn_binary.cpu().numpy(), tst_binary.cpu().numpy(), trn_label.cpu().numpy(), tst_label.cpu().numpy(), config["topK"],config["top_img"])
    ...
    logger.info(f"{config['info']} epoch:{epoch + 1} bit:{config['bit']} dataset:{config['dataset']} MAP:{mAP:.4f} Best MAP: {Best_mAP:.4f}")
