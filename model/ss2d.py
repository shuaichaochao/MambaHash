...
try:
    ...
    from .csms6s import SelectiveScanCore, SelectiveScanOflex
except:
    from csms6s import CrossScan, CrossMerge
    from csms6s import SelectiveScanCore, SelectiveScanOflex
...
class SS2Dv2:
    ...
    def forwardv2(self, x: torch.Tensor, CrossScan, CrossMerge, **kwargs):
        ...
        # x_local = self.conv2d(x)
        # x_local = self.act(x_local)
        # x_global = self.forward_core(x, CrossScan=CrossScan, CrossMerge=CrossMerge)
        # y = x_local.permute(0, 2, 3, 1) + x_global
        # y = self.out_act(y)
